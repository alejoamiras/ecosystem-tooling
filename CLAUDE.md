...
**Live**: all three packages released as `@alejoamiras/*@5.0.1` on npm (`latest`, provenance attestations, attestation-identity verified) — benchmark + private-fee-juice via the tokenless OIDC `release.yml` (mode-based: release/rehearsal/revision; rehearse-then-release choreography with SHA binding + payload comparison), quota-paymaster first-published 2026-08-07 via the one-time since-removed bootstrap mode (its trusted publisher now exists; future releases are tokenless for all three). Consolidation, the 5.0.0 stable migration, the standards-handover/5.0.1 plan (aztec-standards removed, 5.0.1 released, `@alejoamiras/aztec-standards` deprecated, scoped `/harden security medium` on fee-payment), and the quota-fpc-extraction plan (six-round codex post-impl audit: accept) are all COMPLETE (plans are local-only under `implementations-plan/`; audit under `audit/security/`). `docs/roadmap.md` tracks what's next; `docs/ci-pipeline.md` has the full release runbook.
