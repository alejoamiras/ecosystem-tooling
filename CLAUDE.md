...
- `bun run test:nr` — Noir TXE tests per package (`aztec test`); count floors: standards 329, fee-payment 11
...
- **Publish order: benchmark → standards/fee-payment.** rc versions get the `rc` dist-tag, never `latest`; emergency fixes to a published lockstep version ship as `<version>-revision.N` (dist-tag `revision`, `latest` moved manually with OTP).
