...
const FPC = AztecAddress.fromStringUnsafe('0x2f841a5811740dee2a0cb96fe9903312453e1fd5736b8e663105cb3fa95155c2');

const PLAYER = AztecAddress.fromStringUnsafe('0x201d3ae7ed4e9a8c15b29bc5b7c07d660c150b29d86f6317d4cc101885f722b8');

/** A node where the given seat indexes are already claimed. */
function nodeWithTakenSeats(takenIndexes: number[]): SeatProbeNode {
  return {
    async findLeavesIndexes(_block, tree, leaves) {
      ...
      for (const leaf of leaves) {
        if (leaf instanceof Promise || typeof (leaf as { then?: unknown })?.then === 'function') {
...
describe('seat picker', () => {
  test('rejects an invalid maxUsers loudly', () => {
    ...
    expect(() => assertValidMaxUsers(undefined as unknown as number)).toThrow(RangeError);
...
describe('hasSubscribed', () => {
  ...
  test('reports a claimed allowance when the nullifier is present', async () => {
    ...
    expect(await hasSubscribed({ node, fpcAddress: FPC, generation: 20_663, player: PLAYER })).toBe(true);
  });
});
