...
describe('config validation', () => {
  ...
  test('the loss cap accounts for the ~3 generations spendable around rollover', () => {
    ...
    expect(() => parseQuotaFpcConfig(base({ maxLossWei: '300000' }))).not.toThrow();
  });
  ...
  test("values beyond the contract's integer widths are rejected", () => {
    expect(() =>
      parseQuotaFpcConfig(
        base({
          policy: { maxFeeWei: (2n ** 128n).toString(), maxUsesPerDay: 1, maxUsersPerDay: 1 },
        }),
      ),
    ).toThrow(/maximum/);
    expect(() =>
      parseQuotaFpcConfig(
        base({
          policy: { maxFeeWei: '1000', maxUsesPerDay: 2 ** 32, maxUsersPerDay: 1 },
        }),
      ),
    ).toThrow(/u32 maximum/);
  });
  ...
  test('account classes: absent, zero, out-of-field, and duplicate are rejected', () => {
    ...
    expect(() => parseQuotaFpcConfig(base({ allowedAccountClasses: [] }))).toThrow(/at least one account class/);
    // Zero is the contract's empty-slot marker: a named class resolving to it
    // would be silently dropped.
    expect(() =>
      parseQuotaFpcConfig(base({ allowedAccountClasses: [{ name: 'Z', classId: `0x${'0'.repeat(64)}` }] })),
    ).toThrow(/zero/);
    // At/above the field modulus the on-chain value would differ from config.
    expect(() =>
      parseQuotaFpcConfig(base({ allowedAccountClasses: [{ name: 'F', classId: `0x${'f'.repeat(64)}` }] })),
  ...
  test('the unpublished-account requirement defaults ON and rejects non-booleans', () => {
    ...
    expect(parseQuotaFpcConfig(base({ requireUnpublishedAccounts: false })).requireUnpublishedAccounts).toBe(false);
    expect(() => parseQuotaFpcConfig(base({ requireUnpublishedAccounts: 'yes' }))).toThrow(/must be a boolean/);
  });
  ...
  test('adminAddress: absent, zero, malformed, and unresolved env are rejected; env: resolves', () => {
    ...
    expect(() => parseQuotaFpcConfig(base({ adminAddress: undefined }))).toThrow(/adminAddress is required/);
    expect(() => parseQuotaFpcConfig(base({ adminAddress: `0x${'0'.repeat(64)}` }))).toThrow(/zero address/);
    expect(() => parseQuotaFpcConfig(base({ adminAddress: '0x1234' }))).toThrow(/32-byte/);
    ...
    expect(() => parseQuotaFpcConfig(base({ adminAddress: `0x${'f'.repeat(64)}` }))).toThrow(/field element/);
    // env: indirection is explicit, never a fallback: unset var = error.
    expect(() => parseQuotaFpcConfig(base({ adminAddress: 'env:MISSING_ADMIN' }), {})).toThrow(/set MISSING_ADMIN/);
