...
describe('generation arithmetic', () => {
  ...
  test('the reset countdown reaches zero exactly at midnight', () => {
    ...
    expect(millisUntilReset(DAY_START + SECONDS_PER_DAY)).toBe(Number(SECONDS_PER_DAY) * 1000);
  });

  test('generationEndsAt is the following midnight', () => {
    expect(generationEndsAt(generationAt(DAY_START))).toBe(DAY_START + SECONDS_PER_DAY);
  });
  ...
  test('tomorrow is sponsorable only inside the closing grace window', () => {
    ...
    expect(sponsorableGenerations(DAY_START + SECONDS_PER_DAY - 660n)).toEqual([today]);

    // 9 minutes before midnight: tomorrow becomes acceptable.
    expect(sponsorableGenerations(DAY_START + SECONDS_PER_DAY - 540n)).toEqual([today, today + 1]);
  });
  ...
  test('a generation held across midnight goes stale', () => {
    ...
    expect(isGenerationStale(today, DAY_START + SECONDS_PER_DAY + 1n)).toBe(true);
