 ...
 * the nullifier tree.
 *
 * Carries a hard-won guard from the source project: an invalid `maxUsers`
...
export async function takenSeats(query: SeatQuery, seats: number[]): Promise<Set<number>> {
  assertValidMaxUsers(query.maxUsers);
  if (seats.length === 0) return new Set();

  const nullifiers = await siloedSeatNullifiers(query.fpcAddress, query.generation, seats);
  ...
  for (let offset = 0; offset < nullifiers.length; offset += CHUNK) {
    ...
    const found = await query.node.findLeavesIndexes('latest', NULLIFIER_TREE, slice);
 ...
 */
export async function findFreeSeat(query: SeatQuery, random: () => number = Math.random): Promise<number | null> {
  assertValidMaxUsers(query.maxUsers);
  const all = Array.from({ length: query.maxUsers }, (_, i) => i);
...
export async function hasSubscribed(query: {
  ...
  const found = await query.node.findLeavesIndexes('latest', NULLIFIER_TREE, [nullifier]);
