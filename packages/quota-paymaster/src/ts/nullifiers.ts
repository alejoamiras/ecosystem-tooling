...
function assertValidGeneration(generation: number): void {
  if (!Number.isInteger(generation) || generation < 0) {
    throw new RangeError(`Generation must be a non-negative integer, got ${generation}`);
  }
}
 ...
 */
export async function computeSeatNullifier(generation: number, seat: number): Promise<Fr> {
  assertValidGeneration(generation);
  if (!Number.isInteger(seat) || seat < 0) {
    throw new RangeError(`Seat must be a non-negative integer, got ${seat}`);
  }
  return poseidon2HashWithSeparator([new Fr(generation), new Fr(seat)], SEAT_NULLIFIER_SEPARATOR);
}

/** Unsiloed per-user nullifier: proves whether a user subscribed this generation. */
export async function computePlayerNullifier(generation: number, player: AztecAddress): Promise<Fr> {
  assertValidGeneration(generation);
  return poseidon2HashWithSeparator([new Fr(generation), player.toField()], PLAYER_NULLIFIER_SEPARATOR);
}
