...
/** Mirrors the contract's ACCOUNT_MAX_CALLS (aztec-nr's AppPayload capacity). */
...
export interface SandwichWallet {
  createAuthWit(from: AztecAddress, intent: { consumer: AztecAddress; innerHash: Fr }): Promise<AuthWitness>;
}

/** A contract-method interaction able to produce its ExecutionPayload. */
export interface SandwichInteraction {
  request(): Promise<ExecutionPayload>;
}

/** Contract handle exposing the paymaster's two entrypoints. */
export interface QuotaFpcMethods {
  subscribe_and_execute: (...args: unknown[]) => SandwichInteraction;
  sponsor_and_execute: (...args: unknown[]) => SandwichInteraction;
}
...
export async function buildSandwichPayload(
  ...
  const interaction = isFirstOfDay
    ? fpc.methods.subscribe_and_execute(encodedCalls, request.player, request.generation, request.seat)
    : fpc.methods.sponsor_and_execute(encodedCalls, request.player, request.generation);

  const outer = await interaction.request();

  return new ExecutionPayload(
    outer.calls,
    [authWitness, ...outer.authWitnesses],
    [...outer.capsules],
