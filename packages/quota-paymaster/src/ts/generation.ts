...
export function generationAt(chainTimestampSeconds: bigint): number {
  if (chainTimestampSeconds < 0n) {
    throw new RangeError(`Chain timestamp cannot be negative: ${chainTimestampSeconds}`);
  }
 ...
 */
export function sponsorableGenerations(chainTimestampSeconds: bigint): number[] {
  const today = generationAt(chainTimestampSeconds);
  const secondsIntoDay = chainTimestampSeconds % SECONDS_PER_DAY;
  const inGraceWindow = secondsIntoDay >= SECONDS_PER_DAY - ROLLOVER_GRACE_SECONDS;
 ...
 */
export function isGenerationStale(generation: number, chainTimestampSeconds: bigint): boolean {
