...
- **Attestation-identity omission shipped in 5.0.0**: `canonical-deployment.json` is absent from the immutable 5.0.0 fee-payment tarball (added to `files` afterward). It ships with the next version; until then the machine-checked README is the consumer surface. No revision-only release warranted.
